...
class CML(torch.nn.Module):
    def __init__(self, x1_dim, x2_dim, metaGraph_input_dim, CMFF_input_dim, CMFF_output_dim, num_classes, dropout, hgc, lg):
        ...
        for i in range(lg):
            hidden_dim = CMFF_output_dim if i == 0 else hidden_dim_list[i-1]
            # sym是采用对称归一化
            self.GCN.append(tg.nn.ChebConv(hidden_dim, hidden_dim_list[i], K=3, normalization='sym', bias=bias))

        self.GC1 = tg.nn.ChebConv(CMFF_output_dim, CMFF_output_dim, K=3, normalization='sym', bias=bias)
        self.GC2 = tg.nn.ChebConv(CMFF_output_dim, CMFF_output_dim, K=3, normalization='sym', bias=bias)
        self.SGC = tg.nn.ChebConv(CMFF_output_dim, CMFF_output_dim, K=3, normalization='sym', bias=bias)

        self.CMFF = CMFF(x1_dim, x2_dim, CMFF_input_dim, CMFF_output_dim, CMFF_output_dim)

        self.meta_adj_net = metaGraph(input_dim=metaGraph_input_dim//2, dropout=dropout)

        self.attention = Attention(3*CMFF_output_dim, CMFF_output_dim, CMFF_output_dim)
    ...
    def get_adj(self, fusion_feature, metadata):
        '''
        get adj for GCN
        '''
        # number of subjects
        n = fusion_feature.shape[0]
        num_edge = n * n
        ...
        feature_graph = get_feature_graph(fusion_feature)
        ...
        meta_adj_keep_ind = np.where(meta_adj_weight.detach().numpy() > 0.9)[0]
        meta_adj_index = meta_adj_index[:, meta_adj_keep_ind]
        meta_adj_weight = meta_adj_weight[meta_adj_keep_ind]


        adj_keep_ind = np.where(adj_weight > 1.8)[0]
    ...
    def forward(self, x1, x2, x3):
        ...
        features_fusion, atten1, atten2, weight1, weight2 = self.CMFF(x1, x2)
        ...
        return F.softmax(logit, dim=-1), features_fusion, atten1, atten2, weight1, weight2
