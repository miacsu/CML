...
multi-stage multi-modal learning method based concat (SNP and MRI)
...
from models.concatMultiGraphLearningFusion import concatMGLF
...
def main(opt, exp, seed):
    ...
    for fold in range(n_folds):
        ...
        model = concatMGLF(features1.shape[1], features2.shape[1], opt.num_classes, opt.dropout,
                       hgc=opt.hgc, lg=opt.lg, edgenet_input_dim=2*features3.shape[1]).to(opt.device)
        ...
        x3 = torch.tensor(features3, dtype=torch.float32)
