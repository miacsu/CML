...
def main(opt, exp, seed):
    ...
    features = np.loadtxt(os.path.join(data_folder, "MRI.csv"), delimiter=',')
    # features = np.loadtxt(os.path.join(data_folder, "SNP.csv"), delimiter=',')
    # features = np.loadtxt(os.path.join(data_folder, "meta.csv"), delimiter=',')
    y = np.loadtxt(os.path.join(data_folder, "labels.csv"), delimiter=',')

    n_folds = opt.num_fold
    cv_splits = data_split(features, y, n_folds, seed)
    ...
    for fold in range(n_folds):
        ...
        features, support = get_node_features(features, y, train_ind, 300, True)

        # build network architecture
        model = GCN(features.shape[1], opt.num_classes, opt.dropout, opt.hgc, opt.lg)
        ...
        x1 = torch.tensor(features, dtype=torch.float32)
